...
def welcome():
    return (
        f"Welcome to Lorie's 1416 Version of Justice League API!<br/>"
...
def precipitation(cursor, row):
    """Convert the query results to a Dictionary using date as the key and prcp as the value."""
    """Return the JSON representation of your dictionary."""
    con = sqlite3.connect(":memory:")
    con.row_factory = precipitation
    cur = con.cursor()
...
def start_date(input_column):
    ...
    conn = sqlite3.connect("Resources/hawaii.sqlite")
    cur = conn.cursor()
    cur.execute("SELECT min(tobs) from measurement where date >= " + str(input_column))
    rows = cur.fetchall()
    column_list = []
    for row in rows:
        column_list.append(row[0])
